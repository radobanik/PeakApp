...
function SingleSelectDropdown<T extends DropdownOption>({
  options,
  selected,
  label,
  onSelect,
...
function MultiSelectDropdown({ options, selected, onChange }: MultiSelectProps) {
  ...
  const display = selected.length
    ...
    : 'Select'
  return (
    <div className="flex flex-col gap-2 md:col-span-2" ref={ref}>
      <Label>Climbing Structure</Label>
      <Button
        variant="outline"
        onClick={() => setOpen(!open)}
        className="w-full flex flex-wrap justify-start items-center gap-2 h-auto"
      >
        {display}
      </Button>
      {open && (
        <div className="absolute z-10 bg-white border rounded-md shadow-md w-full mt-2 max-h-60 overflow-auto">
          {options.map((option) => (
            <div
              key={option}
              className={`flex items-center gap-2 p-2 cursor-pointer hover:bg-gray-100 ${selected.includes(option) ? 'bg-blue-50' : ''}`}
              onClick={() => toggle(option)}
            >
              {selected.includes(option) && <Check className="w-4 h-4 text-blue-600" />}
              <Badge
                className={`px-2 py-1 ${selected.includes(option) ? climbingStructureStyles[option] : 'bg-gray-200 text-gray-800'}`}
              >
                {option}
              </Badge>
            </div>
