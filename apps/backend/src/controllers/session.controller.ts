...
import { ActivityUnassign, ActivityUpdate } from "../model/activity";
...
const create = async (req : Request<SessionCreate>, res : Response) => {
    ...
    for (let i = 0; i < activities.length; i++) {
        ...
        if (activity.session != null) {
            res.status(HTTP_STATUS.BAD_REQUEST_400).json({ error: "Cannot assign Activity from another Session." });
            return;
        }
    }
